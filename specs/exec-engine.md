...
`(compressedTxSize) * (l1Basefee*lBasefeeScalar + l1BlobBasefeeScalar*l1BlobBasefeeScalar) / 1e6`
...
- `l1BlobBasefee` is the blob gasprice, computed as described in
  [EIP-4844](https://github.com/ethereum/EIPs/blob/master/EIPS/eip-4844.md#gas-accounting) from
  the header of the latest registered L1 origin block.
...
- read from the L1 Block Info contract (`0x4200000000000000000000000000000000000015`)
  ...
  - using direct storage-reads:
    - basefee as big-endian `uint256` in slot `1`
    - blobBasefee as big-endian `uint256` in slot `7`
    - l1BasefeeScalar as big-endian `uint32` in first 4 bytes of slot `8`
    - l1BlobBasefeeScalar as big-endian `uint32` in the second 4 bytes of slot `8`
